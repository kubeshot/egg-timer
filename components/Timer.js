...
const Timer = ({ route }) => {
  ...
  const initialTime = useRef(route.params?.time || 0);
  ...
  useEffect(() => {
    if (route.params) {
      setHeading(route.params.heading || "");
      setSubHeading(route.params.subHeading || "");
      setTime(route.params.time || 0);
      initialTime.current = route.params.time || 0;
    ...
    const timerInterval = setInterval(() => {
      if (!isPaused && time > 0) {
        ...
        setTime((prevTime) => {
          ...
          animateSpokes(1 - newTime / initialTime.current);
          return newTime;
        });
      }
  ...
  const animateSpokes = (progress) => {
    ...
    spokeAnimations.forEach((anim, index) => {
      ...
      Animated.timing(anim, {
        toValue: adjustedIndex < fadedSpokes ? 1 : 0,
        duration: 100, // Shorter duration for smoother transitions
        useNativeDriver: false,
        easing: Easing.linear,
      }).start();
    });
